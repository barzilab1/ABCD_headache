...
univariate_headache_1toall <- vars[1:length(vars)] %>%  
    map_dfr(~get_est(outcome = "medhx_2q_l", predictor = .x, variables = var_headache_0, data = abcd_gr1, estimate = "OR", output = output, conf_int = 0.9))

univariate_headache_1toall <- univariate_headache_1toall %>% 
    mutate(significant = case_when(p_value <= 0.1 ~ "sig", TRUE ~ NA_character_))

writexl::write_xlsx(univariate_headache_1toall, "../outputs/univariate_headache_1toall_111422_abcd.xlsx")
```
...
saveRDS(univariate_plot, file = "../outputs/univariate_plot_abcdgr1_coef_120722.rds")
```
