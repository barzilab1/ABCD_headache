...
```{r}
library(naniar)
# check missing data of 21 sig_vars
## baseline
data_long %>% filter(eventname == "baseline_year_1_arm_1") %>% select(all_of(sig_vars)) %>% miss_var_summary()
data_long %>% filter(eventname == "1_year_follow_up_y_arm_1") %>% select(all_of(sig_vars)) %>% miss_var_summary()
data_long %>% filter(eventname == "2_year_follow_up_y_arm_1") %>% select(all_of(sig_vars)) %>% miss_var_summary()
data_long %>% select(all_of(sig_vars)) %>% miss_var_summary()


table(data_long$eventname)
```
