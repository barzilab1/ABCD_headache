...
```{r}
# vars1 <- dataset %>% dplyr::select(
# # Headache problems
#         medhx_2h_l, # Epilepsy or Seizures
#         medhx_2q_l,	# Very Bad Headaches
#         medhx_6i_l,	# Head Injury
#         medhx_6j_l,	# Knocked Unconscious
# contains(c("Migraine", "Daily.Preventive", "Rescue.Medications")), headache_severity_cat) %>% names()
write.csv(print(CreateTableOne(data = dataset %>% mutate(eventname = factor(eventname, c("baseline_year_1_arm_1", "1_year_follow_up_y_arm_1", "2_year_follow_up_y_arm_1"))), 
